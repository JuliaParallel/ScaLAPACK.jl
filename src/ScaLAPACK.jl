...
const ictxt = Array(Int32, 1) # system context
...
function descinit(m::Integer, n::Integer, mb::Integer, nb::Integer, irsrc::Integer, icsrc::Integer, lld::Integer)

    # extract values
    nprow, npcol, myrow, mycol = blacs_gridinfo()
    ...
    mb > 0 || throw(ArgumentError("first diemnsion blocking factor must be positive"))
    nb > 0 || throw(ArgumentError("second diemnsion blocking factor must be positive"))
